...
function getPostList(){
     ...
     * showOffcanvas 함수에 올바른 postId가 전달됩니다.
     * 클로저개념때문에 진짜 미쳐버리겠음...
     * */
    fetch(url)
        .then(response => response.json())
        .then(response =>{
            ...
            for(let e of response.data.postList){
                ...
                const secondAccountName = newPostWrapper.querySelector(".secondAccountName");
                secondAccountName.innerText = e.postDto.account_name;
...
//검색 로직 추가.. 하지만 가라 UI라서 이후 기능추가 필요
