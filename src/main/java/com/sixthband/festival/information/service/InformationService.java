...
public class InformationService {
    ...
    //rootPath는 final로 고정되어 수정되지않는다
    //여기서 separator는 OS에 맞춘 경로 분기법입니다.
    //File.separator 상수는 운영 체제에 따라 적절한 경로 구분자를 반환한다 (예: Windows에서는 '', Linux에서는 '/').
    //경로 문자열은 여러 문자열을 쉼표로 구분하여 연결. 아래 변수'rootPath'는 다음과 같은 경로를 생성한다. "Users/test/sixthBandImage"
    ...
    //SELECT 구문의 List 객체 데이터를 개별로 분리해줘야함
    ...
    //라인업 삭제하기, 에러에 대한 정의는 따로 진행하지 않음
    public void deleteLineUpAndDeleteDateByid(int id) {
        try {
            ...
            System.err.println(
                    "Error deleting from festival_information_performanceDate for id: " + id + ": " + e.getMessage());
        }
        try {
